...
-- Jos cleanStart on true, niin asetustiedosto luodaan uudelleen aina kun peli käynnistyy.
...
settings.userdata = {
	...
	sfxVolume = 50,
	language = "english",
	font = "Resources/Fonts/Gotu-Regular.ttf",
	fullscreen = false,
...
if not file or cleanStart then
	-- Suljetaan tiedosto jos se on auki, jotta se voidaan avata uudelleen.
	if file then
		io.close( file )
	end
    settings.save()
...
function settings.setAudio()
    ...
    for i = 2, 32 do
        audio.setVolume( settings.userdata.sfxVolume / 100, { channel = i } )
    end
end
