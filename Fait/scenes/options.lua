		...
		if event.phase == "ended" then
			...
			if ( id == "close" ) then
				...
				elseif id == "fullscreen" then
					settings.userdata.fullscreen = not settings.userdata.fullscreen
...
function scene:create( event )
	...
	local title = display.newText( sceneGroup, "Options", window.x, window.y - window.height / 2 + 56, settings.userdata.font, 32 )
	...
	local buttondata = {
		...
		-- resoluutioasetukset
		{ text = "Resolution", id = "resolution", x = 475, y = window.y + window.height / 2 - 150, disabled = true },
	...
	for i = 1, #buttondata do
		local button = widget.newButton({
			...
			isEnabled = not buttondata[i].disabled,
	...
	local sliderdata = {
		...
		-- sfx volume
		{ text = "SFX volume", id = "sfxVolume", x = window.x + window.width / 2 - 120, y = window.y + window.height / 5 - 230, value = settings.userdata.sfxVolume}
	}
	...
	for i = 1, #sliderdata do
		...
		local text = display.newText( sceneGroup, sliderdata[i].text, slider.x + 4, slider.y + slider.height*0.5*slider.yScale, settings.userdata.font, 20 )
		text.anchorX = 1
	end
