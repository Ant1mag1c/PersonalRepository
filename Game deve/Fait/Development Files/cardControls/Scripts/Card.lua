...
function card.newCard( cardType, x, y )
    ...
    newCard.background = display.newImageRect( newCard, "Assets/Images/cardFront.png", cardWidth, cardHeight )

    newCard.image = display.newImageRect(newCard,"Assets/Images/" .. data.image, 65, 65)
    newCard.image.x, newCard.image.y = 0, -cardHeight*0.175

    newCard.frame = display.newImageRect(newCard, "Assets/Images/" .. data.frame, 78, 78 )
    newCard.frame.x, newCard.frame.y = 0, -cardHeight*0.175 - 2

    newCard.energy = display.newImageRect(newCard, "Assets/Images/" .. data.energy, 20 ,20)
    ...
    playerHand[#playerHand+1] = newCard
    return #playerHand
end

--animating the cards on the screen
function card.dealCards()
    for i = 1, #playerHand do
        transition.to(playerHand[i], { x = (i-1) * 90, y = display.contentHeight / 2 + 150, time = 500, delay = (i) * 100 })
    end
end
...
function card.removeCard()
	...
	--removing the active card from player hand
