        ...
        if moveMapDistance < 5 then
            ...
            if target.row == currentRow + 1 then
                ...
                if isConnected then
                    ...
                    target:setFillColor(1,0.9,0)
                    -- print(target.terrain, target.type)
...
local function moveMap(event)
    ...
    elseif event.phase == "moved" then
        ...
        moveMapDistance = math.sqrt( deltaX^2 + deltaY^2 )

        groupMap.x = target.xStart + deltaX
        groupMap.y = target.yStart + deltaY

        if groupMap.x > groupMap.dragMaxX  then
            -- print("left border exdeeded")
            groupMap.x = groupMap.dragMaxX
            initTouch( event )

        elseif groupMap.x < groupMap.dragMinX then
            -- print("right border exdeeded")
            groupMap.x = groupMap.dragMinX
            initTouch( event )
        end

        if groupMap.y < groupMap.dragMinY then
            -- print("bottom border exdeeded")
            groupMap.y = groupMap.dragMinY
            initTouch( event )

        elseif groupMap.y > groupMap.dragMaxY then
            -- print("top border exdeeded")
            groupMap.y = groupMap.dragMaxY
            initTouch( event )
        end

        -- print("")
        -- print("mapX: " .. groupMap.x)
        -- print("mapY: " .. groupMap.y)
...
function map.start( parent, sceneParams )
	if not hasStarted then
        ...
        -- TODO: Laske groupMap.x:n oikea startti piste, ei 0

        -- Kartalle luotu raja jota voi hyväksikäyttää myöhemmin
        -- local mapArea = display.newRect( groupMap, 0, startNode.y + startNode.height*0.5, groupMap.width, groupMap.height )
        -- mapArea.anchorX, mapArea.anchorY = 0, 1
        -- mapArea:setFillColor( 1, 0, 0, 0.25 )

        local background = display.newImageRect( groupMap, "Resources/Images/mapBG.png", groupMap.width, groupMap.height )
        background.x, background.y = 0, startNode.y + startNode.height*0.5
        background.anchorX, background.anchorY = 0, 1
        background:toBack()

        -- Asetetaan kartalle rajat
        -- mapBounds = groupMap.contentBounds
        local dragDistance = 100

        -- TODO: aseta kartalle rajat, eli paljonko sitä saa liikuttaa ja mihin suuntiin (1/2).
        -- TODO: Maksimi arvot tulee määrittää ruudun ja kartan koon perusteella
        groupMap.dragMinX = 0
        groupMap.dragMaxX = groupMap.x + dragDistance
        groupMap.dragMinY = 250
        groupMap.dragMaxY = 1000

        print(groupMap.x, groupMap.y)
        -- local cameraX = math.max( math.min( display.contentCenterX - groupMap.x, xMin ), xMax )
        -- local cameraY = math.max( math.min( display.contentCenterY - groupMap.y, xMin ), xMax )
        -- groupMap.x = cameraX - 200
