...
function M.new(data, dir)
  ...
  function map.getFirstTile( ... )
    -- Note: this function was incorrectly initially written as a regular function, so calling
    -- it as a method would crash it. In order to not break existing games that may use this,
    -- we're checking if the user has called this as a regular function or as a method.
    local property, value

    if select('#', ...) == 3 and type((...)) == "table" then
      -- method call: self, property, value
      property, value = select(2, ...)
    else
      -- function call: property, value
      property, value = ...
    end
  ...
  function map.getAllTiles( ... )
    -- Note: see getFirstTile above.
    local property, value
    if select('#', ...) == 3 and type((...)) == "table" then
      property, value = select(2, ...)
    else
