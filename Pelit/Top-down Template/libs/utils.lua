...
--  License: MIT                                                       --
-------------------------------------------------------------------------
-- This is a simplified and reduced version of the original utils.lua.
...
function display.isValid( object, removeAfterCheck )
	local isValid = false
	if type(object) == "table" and object.width ~= 0 and object.height ~= 0 and not object._isRemoved then
		isValid = true
	end
...
function system.createFolder( folder, directory )
	if type( folder ) ~= "string" then
		print( "WARNING: bad argument #1 to 'createFolder' (string expected, got " .. type( folder ) .. ")." )
		return false
	end
	...
	-- Find all folders in the original folder string and create all subsequent subfolders.
	local path = pathForFile( "", directory )
	local folderList = string.split( folder, "/" )

	for i = 1, #folderList do
		local nextFolder = path  .. "/" .. folderList[i]
		local isDirectory = lfs.attributes( nextFolder, "mode" ) == "directory"

		-- If the folder doesn't yet exist, create it.
		if not isDirectory then
			local success = lfs.chdir( path )
			if success then
				lfs.mkdir( folderList[i] )
			else
				-- If there were any issues, then release any folders created from
				-- LFS' control so that they can be later removed, if needed.
				lfs.chdir( pathForFile( "" ) )
				return false
			end
		end
	...
	-- If the folders were created without issues, then release the folders
	-- from LFS' control so that they can be removed later, if needed.
	lfs.chdir( pathForFile( "" ) )
