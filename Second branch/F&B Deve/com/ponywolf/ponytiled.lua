-- "Hacked and slashed version" of PonyTiled used by Fruillo & Berri.
...
local function tiledProperties(properties)
    if (#properties > 0) and properties[1].name and properties[1].value then
        ...
        for i = 1, #properties do
            if translate then
                if
                    properties[i].type == "string" and (properties[i].name == "text") and
                        (not tonumber(properties[i].value))
                 then
                    if translate then
...
local function snap(map, object, alignment, margin, safe)
    if not object or not object.x or not object.y then
        return nil
    end
...
function M.new(data, dir)
                    ...
                    if gid then
                        ...
                        if type(data) == "table" then
                            ...
                            if physicsData.bodyType then
                                physicsData.filter = collisionData.terrainFilter
                        ...
                        if flip.xy then
                            ...
                            if flip.x then
                                image.xScale = -1 * image.xScale
                            end
                            if flip.y then
                                image.yScale = -1 * image.yScale
                            end
                        end
                    end
                end
            ...
            for j = 1, #layer.objects do
                ...
                if object.gid then
                    ...
                    if gid then
                        ...
                        if image.fillColor then
                            image:setFillColor(decodeTiledColor(image.fillColor))
                        end
                        -- flip it
                        if flip.xy then
                        ...
                        if object.properties.bodyType or layer.properties.bodyType then
                            ...
                            object.properties.filter = collisionData.terrainFilter
                ...
                elseif object.polygon or object.polyline then -- Polygon/line
                    ...
                    if object.polygon then
                        ...
                        -- Käytä setFillColor, sillä alphan tai isVisibile:n muutos vaikuttaa myös stroke efektiin.
                        polygon:setFillColor( 0, 0 )

                        if object.properties.type == "wall" then
                            polygon.stroke = {
                                type = "image",
                                filename = "assets/images/mapStroke.png",
                            }
    ...
    function map:_getAllObjects(property, value)
