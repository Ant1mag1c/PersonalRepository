		...
		if not file then
			...
			for line in file:lines() do
				...
				-- Laadi muista riveistä Lua taulukko.
					...
					----------------------------------------------------------------------------------
					-- Jos rivi ei ole disabloitu Google Sheetsissä, niin lisää se taulukkoon.
					-- Näin pystytään varmistamaan, että Sheetsissä voi olla työnalla olevia kortteja,
					-- vihollisia, yms. ilman että niitä tarvitsee poistaa ennen peliin tuomista.
					----------------------------------------------------------------------------------
					if row.disabled == "FALSE" then
						row.disabled = nil -- Poista disabled arvo, koska sitä ei tarvita pidempään.
